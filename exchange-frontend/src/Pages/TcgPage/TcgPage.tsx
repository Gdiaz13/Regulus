...
// Record TCG card prices by hand (no provider feed yet) and review what is stored.
// The gateway keeps the source metadata so sold/graded prices never mix silently.
const TcgPage = () => {
...
function Header() {
  return (
    <header className={styles.header}>
      ...
      <p className={styles.note}>No TCG provider feed exists yet, so signed-in users record the prices they see. Type, condition, grade, and currency are stored so a sold PSA 9 never mixes with a raw listing.</p>
    </header>
  );
}
