...
public sealed class PredictionAccuracyStore
{
    ...
    public async Task<List<PredictionAccuracyResponse>> ListAsync(string? assetId, int? take)
    {
        ...
        return await ScoredAsync(connection, assetId, take);
    }

    // Rolls the per-prediction scores up per model so Regulas can rank its AIs.
    public async Task<List<ModelAccuracySummary>> SummaryAsync(string? assetId, int? take)
    {
        await using var connection = await _factory.OpenDatabaseConnectionAsync();
        return Summarize(await ScoredAsync(connection, assetId, take));
    }

    private static async Task<List<PredictionAccuracyResponse>> ScoredAsync(DbConnection connection, string? assetId, int? take)
    {
